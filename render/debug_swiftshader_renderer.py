...
class SwiftshaderRenderer():
  ...
  def init_renderer_egl(self, width, height):
    ...
    # egl_display = eglGetPlatformDisplay(EGL_DEFAULT_DISPLAY) #eglGetDisplay(EGL_DEFAULT_DISPLAY)
    major = ctypes.c_long()
    # eglGetPlatformDisplay(0x313F, 'nvidia', None)
    eglQueryDevicesEXT = eglGetProcAddress("eglQueryDevicesEXT")
    print(eglQueryDevicesEXT)
    devices = (EGLDeviceEXT*100)()
    eglQueryDevicesEXT(100, devices, major)

    print(eglQueryString(EGL_NO_DISPLAY, EGL_EXTENSIONS))
    
    eglGetPlatformDisplay(0x010000, 'nvidia0', None)
    # print(EGL_DEFAULT_DISPLAY)
    ...
    for t in [EGL_NATIVE_RENDERABLE, EGL_NATIVE_VISUAL_ID,
      EGL_NATIVE_VISUAL_TYPE, EGL_RENDERABLE_TYPE, EGL_SURFACE_TYPE,
      EGL_BUFFER_SIZE, EGL_RENDERABLE_TYPE, EGL_CONFORMANT]:
      vv = []
      for i in range(num_configs.value):
        val = ctypes.c_long()
        eglGetConfigAttrib(egl_display, configs[i], t, val)
        vv.append(val.value)
      print(t, len(vv), vv)
    egl_config = configs[0] #num_configs.value-1]
  ...
  def render(self, take_screenshot=False, output_type=0):
    with self.render_timer.record():
      self._actual_render()
      glFlush()
      # self.render_timer.toc(log_at=1000, log_str='render timer', type='calls')
    ...
    if take_screenshot:
      if self.modality == 'rgb':
        screenshot_rgba = np.ones((self.height, self.width, 4), dtype=np.uint8)
...
def test_renderer():
  ...
  N = 160;
  r = 3;
  out_dir = os.path.join('tmp', 'test-renderer', utils.get_time_str())
  ...
  for i in range(N):
    ...
    screenshot_rgba, _ = r_obj.render(take_screenshot=True)
    m.append(np.mean(screenshot_rgba*1.))
    # cv2.imwrite('{:s}/b_{:02d}.png'.format(out_dir, i), screenshot_rgba[:,:,:3][:,:,::-1])
    # assert(np.all(np.mean(np.reshape(screenshot_rgba, [-1,3]), axis=0) > 20))
  print(np.mean(m))
