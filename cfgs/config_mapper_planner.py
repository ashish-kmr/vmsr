...
def get_default_args():
  summary_args = utils.Foo(display_iters=20, test_iters=400,
...
class ConfigManager(cm.ConfigManagerSolver):
  ...
  def _task_setup(self):
    args = [('batch_size', 'bs4'), ('noise', 'N0'), ('step_size', '128'),
    ('base_resolution', 'br0x125'), ('num_samples', 'ns10'), ('extent_samples', 'es8e2'), 
  ...
  def _task_hook(self, task_vars_args, task_str, args):
    ...
    fovs = [int(extent_samples*2/5*base_resolution)+1]
    vs = [base_resolution]
    ...
    mapper_task_params = me.get_mapper_task_params(batch_size=batch_size,
      ...
      add_flips=add_flips, mapper_noise=mapper_noise, output_optimal_actions=True)
    ...
    task.env_class_2 = me.MapperPlannerEnv
