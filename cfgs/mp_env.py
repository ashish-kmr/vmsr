...
class Building():
  def __init__(self, dataset, name, robot, env, flip=False):
    ...
    materials_scale = 1.0 if FLAGS.pre_render else 0.25
    #materials_scale = 1.0 
    ...
    room_dims = self._get_room_dimensions(env_paths['room_dimension_file'], env.resolution, map.origin, flip=flip)
    room_regex='^((?!hallway).)*$'
  ...
  def render_views(self, nodes, perturb=None, aux_delta_theta=0.):
    # List of nodes to render.
    ...
    for i in range(len(nodes)):
      ...
      #(xyt[2]+perturb[i,2]+aux_delta_theta) * (self.robot.delta_theta)
      #print(xyt[2],perturb[i,2],aux_delta_theta,self.robot.delta_theta)
      nxy = np.array([xyt[0]+perturb[i,0], xyt[1]+perturb[i,1]]).reshape(1, -1)
      nxy = nxy * self.map.resolution
      nxy = nxy + self.map.origin
      camera_xyz = np.zeros((1, 3))
...
class expert_navigator():
  ...
  def get_frames(self,states, lim=None):
  ...
  def plot(self,traj,goal,goal_dist=None,actions=None,only_top=False,add_str='',plot_img=None):
...
class MPDiscreteEnv(tle.DiscreteEnv):
  ...
  def render_views(self, states):
    ...
    imgs = self.task.building.render_views(states_flipped)
    views = [np.array(imgs)]
    return views
  ...
  def take_action(self, states, actions, sim=False):
    ...
    for i in range(batch_size):
      ...
      #print('out',out_state)
      #print('limits',boundary_limits)
      #print(np.array(np.nonzero(self.task.road)).shape)
      ...
      if action == 3:
        ...
        #print('out_state',out_state,action,state,angle_value[action])
      #print('truth values:', self.task.road[int(out_state[0]),int(out_state[1])],(np.array(out_state[0:2])<np.array(boundary_limits)).all(),(np.array(out_state[0:2])>=np.array([0,0])).all())
      #print('')
...
class MPDiscreteEnvGrid(tle.DiscreteEnv):
  """Observation is the first person view of the environment.
  Actions are simple grid world actions.
    - Rotate left, right, move straight stay in place.
    - With some probability it stays in place.
  ...
  def render_views(self, states):
    # Renders out the view from the state.
    if FLAGS.pre_render:
      if self.task.rendered_imgs is not None:
        views = [self.task.rendered_imgs[states,...]*1.]
      elif self.task.rendered_imgs_handle is not None:
        views = []
        for s in states:
          if randperm:
            assert(not randsample)
            views.append(self.task.rendered_imgs_handle['img{:06d}'.format(self.task.shuffle_ind[s])][:]*1)
          elif randsample:
            assert(not randperm)
            num = self.task.graph.nodes.shape[0]
            pick = self.task.randsample_rng.choice(num)
            views.append(self.task.rendered_imgs_handle['img{:06d}'.format(pick)][:]*1)
          else:
            views.append(self.task.rendered_imgs_handle['img{:06d}'.format(s)][:]*1)
        views = [np.array(views)]
    else:
      imgs = self.task.building.render_nodes(self.task.graph.nodes[states, :])
      views = [np.array(imgs)]
    return views
