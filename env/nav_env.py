...
class VisualNavigationEnv(NavigationEnv):
  ...
  def __init__(self, robot, env, task_params, category_list=None,
    ...
    self.task.render_cache_file_handle = None
    self.task.render_cache_file = None
  
  def prepare_for_caching(self, cache_dir):
    # Check if on the fly perturbation is happening.
    logging.error('Prepare_for_caching.')
    assert(self.task_params.data_augment.delta_angle == 0.0), 'delta_xy should be zero'
    assert(self.task_params.data_augment.delta_xy == 0.0), 'delta_angle should be zero'
    
    # Render and save images so tha we don't need a renderer.
    self.load_building_into_scene()
    logging.error('  Rendering all nodes.')
    imgs = self.render_nodes([tuple(x) for x in self.task.nodes])
    self.task.render_cache_file = os.path.join(cache_dir, 'image_cache.h5')

    logging.error('  Saving all renders.')
    with h5py.File(self.task.render_cache_file, 'w') as f:
      for i, img in enumerate(imgs):
        f.create_dataset('img{:06d}'.format(i), data=img)
    
    # Remove things that don't need to be stored the actual mesh and materials.
    logging.error('  Removing r_obj.') 
    self.r_obj = None
  ...
  def _get_render_from_cache(self, node_ids):
    assert(self.task.render_cache_file is not None), 'render_cache_file name is None.'
    if self.task.render_cache_file_handle is None:
      self.task.render_cache_file_handle = h5py.File(self.task.render_cache_file, 'r')
    imgs = []
    for n in node_ids: 
      img = self.task.render_cache_file_handle['img{:06d}'.format(n)][:]*1
      imgs.append(img)
    return imgs
  ...
  def get_features(self, current_node_ids, step_number):
    ...
    if self.task_params.outputs.images:
      ...
      if self.r_obj is None:
        # Nodes are rendered and cached, call function that loads nodes from
        # the cache.
        imgs = self._get_render_from_cache(current_node_ids)
      else:
        # Render nodes and read them.
  ...
  def __del__(self):
    if hasattr(self.task, 'render_cache_file_handle') and self.task.render_cache_file_handle is not None:
      self.task.render_cache_file_handle.close()
...
class BuildingMultiplexer():
  ...
  def get_r_obj(self, camera_param):
    if self.r_obj is None:
      ...
      if not hasattr(cp, 'fov_vertical'): fov_vertical = cp.fov
      else: fov_vertical = cp.fov_vertical
      r_obj.init_display(width=cp.width, height=cp.height, 
        fov_vertical=fov_vertical, fov_horizontal=cp.fov, 
  ...
  def _setup_mapper(self):
    ...
    wt = []
    for i, building_name in enumerate(self.building_names):
      hashes = utils.get_hashes(['class', 'env', 'robot', 'task_params', 'camera_param'], 
        [self.env_class, self.env, self.robot, self.task_params, self.camera_param])
      hashes += ['flip-' + str(self.flip[i]), 'name-' + building_name]
      dir_name = os.path.join(*hashes)
      logging.error('dir_name for caching: %s', dir_name)
      if use_cache:
        try:
          b = utils.load_variables(os.path.join(cache_dir, dir_name, 'cache-data.pkl'))['building']
        except:
          r_obj = self.get_r_obj(self.camera_param)
          utils.mkdir_if_missing(os.path.join(cache_dir, dir_name))
          b = self.env_class(robot=self.robot, env=self.env,
                             task_params=self.task_params,
                             building_name=building_name, flip=self.flip[i],
                             logdir=self.logdir, building_loader=self.dataset,
                             r_obj=r_obj)
          b.prepare_for_caching(os.path.join(cache_dir, dir_name))
          utils.save_variables(os.path.join(cache_dir, dir_name, 'cache-data.pkl'), 
            [b], ['building'], True)
      else:
        r_obj = self.get_r_obj(self.camera_param)
        b = self.env_class(robot=self.robot, env=self.env,
